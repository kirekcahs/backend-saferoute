...
export const getCenters = async (req, res) => {
  try {
    const centers = await EvacuationCenter.find({ isActive: true }).sort({
      currentOccupancy: 1,
    }); // least occupied first
...
export const getNearestCenter = async (req, res) => {
  ...
  try {
    ...
    if (!centers.length) {
      return res
        .status(404)
        .json({ message: "No active evacuation centers found" });
    }
    ...
    if (!centersWithDistance.length) {
      return res
        .status(404)
        .json({ message: "All evacuation centers are full" });
    }
